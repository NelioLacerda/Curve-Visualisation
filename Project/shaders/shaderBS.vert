...
// Function for cubic B-spline interpolation
vec2 interpolateBSpline(vec2 p0, vec2 p1, vec2 p2, vec2 p3, float t) {
    float t2 = t * t; // Calculate t squared
    float t3 = t2 * t; // Calculate t cubed

    // Calculate the interpolated position based on the B-spline formula
    return (1.0f / 6.0f) * ((-t3 + 3.0f * t2 - 3.0f * t + 1.0f) * p0 +
        (3.0f * t3 - 6.0f * t2 + 4.0f) * p1 +
        (-3.0f * t3 + 3.0f * t2 + 3.0f * t + 1.0f) * p2 +
        (t3) * p3);
}
...
void main() {
    ...
    if(segmentIndex + 3u < MAX_CONTROL_POINTS) {
        ...
        v_position = interpolateBSpline(p0, p1, p2, p3, t);
